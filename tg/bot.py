...
class Bot(telegram.Bot):
    ...
    def greet(self, update:Update, context:CallbackContext):
        ...
        if not chat_id in self.users.keys() or self.users[chat_id].settings == None:
            ...
            update.message.reply_text(text, reply_markup=self.reply_markup([NEXT_BUTTON]))
    ...
    def settings_respond(self, update:Update, context:CallbackContext):
        ...
        if self.can_save(chat_id, response, settings=True):
            self.save_settings(chat_id, response)
            self.unpin_all_chat_messages(chat_id=chat_id)
            self.pin_chat_message(chat_id=chat_id, message_id=update.message.message_id)
        else:
    ...
    def user_screen(self, chat_id, response):
        next_question = None
        print(self.users[chat_id].type_)
        print('Raw answers:', self.users[chat_id].raw_answers)
        print('Answers:', self.users[chat_id].answers)
        if response == RETURN_BUTTON:
            print('History:', self.users[chat_id].num_questions)
        ...
        elif self.users[chat_id].raw_answers != None and self.users[chat_id].type_ != 'multiple' and response in self.users[chat_id].answers:  ## Checking if next question makes a leap if yes then set it to the current state
    ...
    def save(self, chat_id, response):
        ...
        if self.users[chat_id].type_ == 'multiple':
            ...
            if self.users[chat_id].type_ != 'open' and self.users[chat_id].type_ != 'integer': 
                answer_id = [answer_id]
